...
class DriverQualificationTest(InstrumentDriverQualificationTestCase, TRHPHMixinSub):
    ...
    def test_direct_access_telnet_mode(self):
     ...
     #   time.sleep(0.1)
     #   result = self.tcp_client.expect(Prompt.MAIN_MENU, sleep_time=8)
     #   self.assertTrue(result)
     #   log.debug(">>Successfully sent return and got Command prompt")

        # Select Menu option 2 to change data collection parameters
        log.debug(">>Ready to send option 2 to change data collection parameters ")
        self.tcp_client.send_data("2\r")
        log.debug(">>sent 2 to change data collection parameters ")
        time.sleep(3)
        log.debug(">>Waiting for CHANGE_PARAM_MENU prompt ")
        result = self.tcp_client.expect(Prompt.CHANGE_PARAM_MENU, sleep_time=8)
        self.assertTrue(result)
        ...
        result = self.tcp_client.expect(Prompt.CYCLE_TIME_PROMPT, sleep_time=8)
        self.assertTrue(result)
        ...
        self.tcp_client.send_data("1\r")
        time.sleep(3)
        result = self.tcp_client.expect(Prompt.CYCLE_TIME_SEC_VALUE_PROMPT, sleep_time=8)
        self.assertTrue(result)
    ...
    ###
    def test_direct_access_telnet_mode_old(self):
        """
