...
from mi.dataset.parser.mopak_o_stc import MopakOStcParser, StateKey
from mi.dataset.parser.mopak_o_stc import MopakOStcAccelParserDataParticle, MopakOStcRateParserDataParticle
...
@attr('UNIT', group='mi')
class MopakOStcParserUnitTestCase(ParserUnitTestCase):
    """
    ...
    def setUp(self):
        ...
        self.config = {
            ...
            DataSetDriverConfigKeys.PARTICLE_CLASS: ['MopakOStcAccelParserDataParticle',
        ...
        self.timestamp1 = self.timer_to_timestamp(b'\x00\x00\x82\xb0', start_time_utc)
        self.particle_a_accel = MopakOStcAccelParserDataParticle(b"\xcb\xbd\xe6\xac<\xbd\xd9\nA\xbf\x83\xa4" \
	...
	self.timestamp2 = self.timer_to_timestamp(b'\x00\x00\x9b\x1a', start_time_utc)
	self.particle_b_accel = MopakOStcAccelParserDataParticle(b"\xcb\xbe\x17Q\x8e\xbd\xc7_\x8a\xbf\x85\xc3" \
	...
	self.timestamp3 = self.timer_to_timestamp(b'\x00\x00\xb3\x84', start_time_utc)
	self.particle_c_accel = MopakOStcAccelParserDataParticle(b"\xcb\xbe1\xeak\xbd\xae?\x8a\xbf\x86\x18" \
	...
	self.timestamp4 = self.timer_to_timestamp(b'\x00\x00\xcb\xee', start_time_utc)
	self.particle_d_accel = MopakOStcAccelParserDataParticle(b"\xcb\xbe8\xed\xf0\xbd\xa7\x98'\xbf\x88" \
	    "\x0e\xca\xbd\xeegZ<\xf63\xdc\xbd\xe6b\x8d>U\xa5U>l6p>\xe9\xfc\x8d\x00\x00\xcb\xee\x16e",
	    internal_timestamp=self.timestamp4)
	self.timestamp5 = self.timer_to_timestamp(b'\x00\x00\xe4X', start_time_utc)
	self.particle_e_accel = MopakOStcAccelParserDataParticle(b"\xcb\xbe9t\xb5\xbd\x89\xd1\x16\xbf\x87" \
        ...
        self.timestamp6 = self.timer_to_timestamp(b'\x04ud\x1e', start_time_utc)
        self.particle_last_accel = MopakOStcAccelParserDataParticle(b"\xcb=\xfd\xb6?=0\x84\xf6\xbf\x82\xff" \
        ...
        self.timestampa11 = self.timer_to_timestamp(b'\x00\xae\x18\xb2', start_time_utc)
        self.particle_a11_accel = MopakOStcAccelParserDataParticle(b"\xcb?(\xf4\x85?.\xf6k>\x9dq\x91\xba7r" \
        ...
        self.timestampa1 = self.timer_to_timestamp(b'\x00\xb5\xa1h', start_time_utc)
        self.particle_a1_accel = MopakOStcAccelParserDataParticle(b"\xcb?(\xd3d?/\x0bd>\x9dxr\xba$eZ\xbbl" \
            "\xaa\xea:\xed\xe7\xa6\xbe\xfb\xe1J\xbd\xc6\xfa\x90\xbe\x7f\xcc2\x00\xb5\xa1h\x16\x01",
            internal_timestamp=self.timestampa1)
        self.timestampb1 = self.timer_to_timestamp(b'\x00\xb5\xb9\xd2', start_time_utc)
        self.particle_b1_accel = MopakOStcAccelParserDataParticle(b"\xcb?))$?/(\x9b>\x9e\x15w\xb9\x92\xc0" \
        ...
        self.timestamp1r = self.timer_to_timestamp(b'\x00\xd0\xe7\xd4', start_time_utc)
        self.particle_a_rate = MopakOStcRateParserDataParticle(b"\xcf\xbf\xffNJ?:\x90\x8b@\x1e\xde\xa8\xba" \
            "\tU\xe8\xbb\x07Z\xf2:\xb8\xa9\xc7\x00\xd0\xe7\xd4\x0f\x98", internal_timestamp=self.timestamp1r)
        self.timestamp2r = self.timer_to_timestamp(b'\x00\xd1\x00>', start_time_utc)
        self.particle_b_rate = MopakOStcRateParserDataParticle(b"\xcf\xbf\xffD\xa1?:\x92\x85@\x1e\xde\xcc:\xa3" \
            "6\xf1\xba\xf7I@;\xc4\x05\x85\x00\xd1\x00>\r\xe0", internal_timestamp=self.timestamp2r)
        self.timestamp3r = self.timer_to_timestamp(b'\x00\xd1\x18\xa8', start_time_utc)
        self.particle_c_rate = MopakOStcRateParserDataParticle(b"\xcf\xbf\xffC\xcb?:\x8dL@\x1e\xdf6\xb9\xf5" \
            "\xb1:\xb9\xdf\x06\n;\x05\\a\x00\xd1\x18\xa8\r/", internal_timestamp=self.timestamp3r)
        # last in first_rate file
        self.timestamp4r = self.timer_to_timestamp(b'\x00\xd11\x12', start_time_utc)
        self.particle_d_rate = MopakOStcRateParserDataParticle(b"\xcf\xbf\xffF/?:\x8a\x1d@\x1e\xe0.\xba\xd3" \
            "9*\xba\x80\x1c{:?-\xe9\x00\xd11\x12\x0b\xf2", internal_timestamp=self.timestamp4r)

        # last in larger file
        self.timestamp8r = self.timer_to_timestamp(b'\x00\xd73(', start_time_utc)
        self.particle_last_rate = MopakOStcRateParserDataParticle(b"\xcf\xbf\xffK ?:r\xd4@\x1e\xf4\xf09\xa7\x91" \
    ...
    def timer_to_timestamp(self, timer, start_time_utc):
        """
        ...
        offset_secs = float(int(fields[0]))/62500.0
