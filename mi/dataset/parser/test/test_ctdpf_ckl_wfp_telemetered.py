...
"""
@package mi.dataset.parser.test.test_ctdpf_ckl_wfp_telemetered
@file marine-integrations/mi/dataset/parser/test/test_ctdpf_ckl_wfp_telemetered.py
@author cgoodrich
@brief Test code for a ctdpf_ckl_wfp_telemetered data parser
"""
import os
import struct
import ntplib
...
from mi.dataset.parser.ctdpf_ckl_wfp_telemetered import CtdpfCklWfpParserTelemetered, DataParticleType
from mi.dataset.parser.ctdpf_ckl_wfp_telemetered import CtdpfCklWfpDataParticleTelemetered,\
    CtdpfCklWfpMetadataParticleTelemetered
...
@attr('UNIT', group='mi')
class CtdpfCklWfpParserTelemeteredUnitTestCase(ParserUnitTestCase):
    ...
    """
    ctdpf_ckl_wfp_telemetered Parser unit test suite
    """
    ...
    def setUp(self):
        ...
        self.config = {
            DataSetDriverConfigKeys.PARTICLE_MODULE: 'mi.dataset.parser.ctdpf_ckl_wfp_telemetered',
            DataSetDriverConfigKeys.PARTICLE_CLASS: ['CtdpfCklWfpDataParticleTelemetered',
                                                     'CtdpfCklWfpMetadataParticleTelemetered']
            }
        ...
        self.particle_meta = CtdpfCklWfpMetadataParticleTelemetered((b"\x52\x4e\x75\x82\x52\x4e\x76\x9a", 3.0),
            internal_timestamp=self.start_timestamp)
        self.start_timestamp_long = self.calc_timestamp(start_time, time_increment_270, 0)
        self.particle_meta_long = CtdpfCklWfpMetadataParticleTelemetered((b"\x52\x4e\x75\x82\x52\x4e\x76\x9a", 270.0),
            internal_timestamp=self.start_timestamp_long)

        self.particle_a = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x88\x03\xe3\x3b\x00\x03\xeb\x0a\xc8",
                                                          internal_timestamp=self.start_timestamp)
        self.particle_a_long = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x88\x03\xe3\x3b\x00\x03\xeb\x0a\xc8",
                                                               internal_timestamp=self.start_timestamp_long)
        self.timestamp_2 = self.calc_timestamp(start_time, time_increment_3, 1)
        self.particle_b = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x8c\x03\xe2\xc0\x00\x03\xeb\x0a\x81",
                                                          internal_timestamp=self.timestamp_2)
        self.timestamp_2_long = self.calc_timestamp(start_time, time_increment_270, 1)
        self.particle_b_long = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x8c\x03\xe2\xc0\x00\x03\xeb\x0a\x81",
                                                          internal_timestamp=self.timestamp_2_long)
        timestamp_3 = self.calc_timestamp(start_time, time_increment_3, 2)
        self.particle_c = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x90\x03\xe1\x5b\x00\x03\xeb\x0a\x65",
                                                          internal_timestamp=timestamp_3)
        timestamp_last = self.calc_timestamp(start_time, time_increment_270, 269)
        self.particle_last = CtdpfCklWfpDataParticleTelemetered(b"\x00\x1a\x8f\x03\xe5\x91\x00\x03\xeb\x0bS",
    ...
    def test_simple(self):
        ...
        """
        stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA)
        self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, stream_handle,
                                        self.state_callback, self.pub_callback, self.exception_callback,
                                        len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA))
        # next get records
    ...
    def test_simple_pad(self):
        ...
        Assert that the results are those we expected.
        """
        stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_PAD)
        self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, stream_handle,
                                        self.state_callback, self.pub_callback, self.exception_callback,
                                        len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_PAD))
        # next get records
        result = self.parser.get_records(1)
        self.assert_result(result, 0, self.particle_meta, False, 0, True)
        result = self.parser.get_records(1)
    ...
    def test_get_many(self):
        ...
        Assert that the results are those we expected.
        """
        self.stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA)
        self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, self.stream_handle,
                                        self.state_callback, self.pub_callback, self.exception_callback,
                                        len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA))
        # next get records
    ...
    def test_mid_state_start(self):
        ...
        self.stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA)
        self.parser =  CtdpfCklWfpParserTelemetered(self.config, new_state, self.stream_handle,
                                        self.state_callback, self.pub_callback, self.exception_callback,
                                        len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA))
    ...
    def test_bad_time_data(self):
        ...
        with self.assertRaises(SampleException):
            stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_TIME)
            self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, stream_handle,
                                            self.state_callback, self.pub_callback, self.exception_callback,
                                            len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_TIME))
    ...
    def test_bad_size_data(self):
        ...
        """
        with self.assertRaises(SampleException):
            stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_SIZE)
            self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, stream_handle,
                                            self.state_callback, self.pub_callback, self.exception_callback,
                                            len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_SIZE))
    ...
    def test_bad_eop_data(self):
        ...
        """
        with self.assertRaises(SampleException):
            stream_handle = StringIO(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_EOP)
            self.parser =  CtdpfCklWfpParserTelemetered(self.config, self.start_state, stream_handle,
                                            self.state_callback, self.pub_callback, self.exception_callback,
                                            len(CtdpfCklWfpParserTelemeteredUnitTestCase.TEST_DATA_BAD_EOP))
