...
class IntegrationTest(DataSetIntegrationTestCase):
    ...
    def test_get(self):
        ...
        self.assert_data(DostaLnWfpSioMuleParserDataParticle, 'first.result.yml', count=3)
            
        self.create_sample_data('node58p1_1st2WE.dat', 'node58p1.dat')
        self.assert_data(DostaLnWfpSioMuleParserDataParticle, 'second.result.yml', count=1)

    def test_update_before_retrieval(self):
        """
        Test that we can get data from files.  Verify that the driver
        sampling can be started and stopped
        """

        log.debug("                  ************************************************   test_update_before_retrieval")
        # Start sampling and watch for an exception. node58p1_1st2WE.dat contains the first 2 WE blocks in node58p1.dat
...
class QualificationTest(DataSetQualificationTestCase):
    ...
    def test_stop_start(self):
        ...
        try:
            ...
            self.create_sample_data('node58p1_1st2WE.dat', 'node58p1.dat')
            # Now read the first record of the second WE then stop
            ...
            # Restart sampling and ensure we get the next record, first record of the 2nd WE sio mule header.
            self.assert_start_sampling()
            result3 = self.get_samples(DataParticleType.SAMPLE, 1)
    ...
    def test_shutdown_restart(self):
        ...
        try:
            ...
            # Read the second file, get the first record, then stop
            self.create_sample_data('node58p1_1st2WE.dat', 'node58p1.dat')
            result2 = self.data_subscribers.get_samples(DataParticleType.SAMPLE, 1)
            self.assert_stop_sampling()
            self.assert_sample_queue_size(DataParticleType.SAMPLE, 0)
